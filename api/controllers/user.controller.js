...
export const test = (req, res) => {
  res.json({
    message: 'las rutas de api estan funcionando',
  });
};
...
export const deleteUser = async (req, res) => {
  if (req.user.id !== req.params.id)
    return next(
      errorHandler(401, 'Sólo puedes eliminar una cuenta que sea tuya.')
    );
  try {
...
export const getUserListings = async (req, res, next) => {
  if (req.user.id === req.params.id) {
    ...
    return next(errorHandler(401, 'Sólo puedes ver tus propiedades!'));
  }
};
